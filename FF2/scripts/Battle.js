...
{	
	...
	var menuEnemySelect = new Menu(); {
		menuEnemySelect.Initialize = function()
		{
			...
			for (var i = 0; i < enemies.length; i++)
				this.AddItem(enemies[i].Entity, function() { }, Color.White);
		...
		{
			DrawTextCenter(Game.ScreenWidth / 2, 168, Game.FontMain, Color.White, this.items[this.Selection].item.Name);
		}
	...
	var menuAttack = new Menu(); {
		...
		{			
			for (var i = 0; i < activeEntity.Entity.BattleCommands.length; i++)
				this.AddItem(activeEntity.Entity.BattleCommands[i], function() { }, Color.White);
			
			
			this.AddItem(activeEntity.Entity.BattleCommands[0], function() { }, Color.White);
			this.AddItem(activeEntity.Entity.BattleCommands[0], function() { }, Color.White);
			this.AddItem(activeEntity.Entity.BattleCommands[0], function() { }, Color.White);
			this.AddItem(activeEntity.Entity.BattleCommands[0], function() { }, Color.White);
			this.AddItem(activeEntity.Entity.BattleCommands[0], function() { }, Color.White);
	...
	this._Render = function()
	{
		...
		{
			// Hp/Mp rectangles
			Rectangle(144, 203 + 16 * i, 60, 2, Color.DarkRed);
			Rectangle(144, 203 + 16 * i, 60, 1, Color.Black);
			Rectangle(207, 203 + 16 * i, 30, 2, Color.DarkRed);
			Rectangle(207, 203 + 16 * i, 30, 1, Color.Black);
			
			// Gradients
			if (party[i].Entity.Hp > 0)
				GradientRectangle(144, 203 + 16 * i, (party[i].Entity.Hp / party[i].Entity.MaxHp) * 60, 2, Color.HpBlue, Color.White, Color.White, Color.HpBlue);
				
			if (party[i].Entity.Mp > 0)
				GradientRectangle(207, 203 + 16 * i, (party[i].Entity.Hp / party[i].Entity.MaxHp) * 30, 2, Color.MpGreen, Color.White, Color.White, Color.MpGreen);
			...
			Rectangle(280, 196 + 16 * i, party[i].CurrentTime / party[i].MaxTime * 32, 5, Color.Yellow);
		}
	}
	...
	this._PostRender = function()
	{
		...
		DrawText(14, 188, Game.FontBattleSmall, Color.LightGrey, "name");
		DrawText(93, 188, Game.FontBattleSmall, Color.LightGrey, "barrier");
		DrawText(144, 188, Game.FontBattleSmall, Color.LightGrey, "hp");
		DrawText(210, 188, Game.FontBattleSmall, Color.LightGrey, "mp");
		DrawText(240, 188, Game.FontBattleSmall, Color.LightGrey, "limit");
		DrawText(279, 188, Game.FontBattleSmall, Color.LightGrey, timeMode);
		...
		for (var i = 0; i < party.length; i++)
		{
			...
			if (activeEntity != null && activeEntity.Entity.Name == party[i].Entity.Name && highlightName)
				DrawText(14,  194 + 16 * i, Game.FontMain, Color.Grey, party[i].Entity.Name);
			
			// HP
			DrawTextRight(172, 194 + 16 * i, Game.FontBold, fontColor, party[i].Entity.Hp);
			DrawText(172, 194 + 16 * i, Game.FontBold, Color.Grey, "/");
		...
		// Debug
			// Enemies Stuff
		for (var i = 0; i < enemies.length; i++)
			DrawText(10, 20 + i * 15, GetSystemFont(), Color.White, enemies[i].Entity.Name + ": " + enemies[i].Entity.Hp);		
	}
